...
$(function () {
    ...
    $('#btnLogin').click(function (e) {
        ...
        if (username.trim() === '' || pass.trim() === '') {
            showDialog('You are missing one or more required values.', 'ERPS');
        }
        else {
            $.ajax({
                type: 'GET',
                ...
                success: function (response) {
                    if (response == null || response.Error === "NOTFOUND") {
                        showDialog('Sorry, we were unable to find your information using those credentials. Please check your entries and try again.', 'ERPS');
                    }
                    else if (response.Error == "INVALID") {
                        showDialog('Incorrect login or password. Please check your entries and try again.', 'ERPS');
                    }
                        ...
                        showDialog("Missing or invalid field: " + response.Error, 'ERPS');
                    }
                    else if (response.ID == 0) {
                        showDialog("Error: " + JSON.stringify(response));
                    }
                    else {
                ...
                error: function (response) {
                    showDialog('Sorry, a technical issue prevented us from finding your information. Please try again later.', 'ERPS');
                }
            });
        }
    });
        ...
        success: function (data) {
            if (data == null || data.length == 0) {
                ...
                if (current == 'CallerName') {
                    ...
                    let selectedOption = document.querySelector('input[name="customers"]:checked').value;
...
function saveCall(current, next, user) {
    ...
    if (current == 'ClaimEndCall' || current == 'EndCall') {
        thisCall = null;
    }
