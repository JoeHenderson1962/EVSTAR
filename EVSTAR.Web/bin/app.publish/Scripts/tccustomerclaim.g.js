...
function getClientCode() {
    let clientCode = "TechCycle";
    let customer = sessionStorage.getItem('Customer');
    if (customer != null && customer != undefined && customer != 'undefined') {
        customer = JSON.parse(customer);
        if (customer != null && customer.ID > 0 && customer.CustomerClient != null) {
            clientCode = customer.CustomerClient.Code;
        }
    }

    if (clientCode == "TechCycle" || clientCode == "") {
        let curPage = window.location.pathname.toUpperCase();
        let curHost = window.location.host.toUpperCase();
        isDobson = (curHost.indexOf("DOBSON.") >= 0); // || curHost.indexOf("LOCALHOST") >= 0);
        isReach = (curHost.indexOf("REACH") >= 0); // || curHost.indexOf("LOCALHOST") >= 0);
        isEVSTAR = (curHost.indexOf("WWW.GOEVSTAR") >= 0);
...
$(function () {
    ...
    if (clientCode == 'DOB' && curPage == "/DEFAULT.ASPX") {
    ...
    if (clientCode == 'EVSTAR') {
        clientProgram = 'EVSTAR';
    ...
    function saveCustomer() {
        ...
        $.ajax({
            ...
            headers: { "clientCode": thisCustomer.CustomerClient.Code },
            ...
            success: function (data) {
                if (data != null) {
                    if (data.toUpperCase().indexOf('INVALID DATA') > -1) {
                        ...
                        clientCode = data.CustomerClient.Code;
    ...
    if (curPage.includes("/DOBSONPORTAL") || curPage.includes("\\DOBSONPORTAL")) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response == null || (response.Result != null && response.Result.includes("NOTFOUND"))) {
                        ...
                        clientCode = response.CustomerClient.Code;
        ...
        clientProgram = isReach ? 'REACH' : (isDobson ? 'DOB' : (isEVSTAR ? 'EVSTAR' : 'Techcycle'));
        //if (client == null)
        //    GetClientInfo(clientProgram);
    ...
    if (curPage.includes("/LOGIN") || curPage.includes("\\LOGIN")) {
        ...
        if (thisClient != null) {
            thisClient = JSON.parse(thisClient);
            isReach = (thisClient.Code == 'REACH');
            isDobson = (thisClient.Code == 'DOB');
            isEVSTAR = (thisClient.Code == 'EVSTAR');
        }
        ...
        else if (isDobson || isEVSTAR) {
            $('#divLoginWithText').html('Log in with your email address and password.');
            $('#loginType').html('Email Address');
            $('#loginTypeDescription').html('The email address on your plan');
        }
        else {
            $('#divLoginWithText').html('Log in with your email address or mobile number and password.');
            $('#loginType').html('Email Address or Mobile Number');
            $('#loginTypeDescription').html('The email address or mobile # on your plan');
        }
        ...
        $('#btnLogin').click(function (e) {
            ...
            if (email.trim() === '' || pass.trim() === '') {
                ...
                $.ajax({
                    type: 'GET',
                    url: "/api/customer",
                    headers: { "email": email, "phone": email, "password": pass, "login": isReach ? "mdn" : "email", "clientCode": clientCode },
                    contentType: 'application/json; charset=utf-8',
                    success: function (response) {
                        if (response == null || response.ID == 0 || (response.Result != null && response.Result.includes("NOTFOUND"))) {
    ...
    else if (curPage.includes("/CHARGERQUOTE.") || curPage.includes("\\CHARGERQUOTE.")) {
        //$("#txtCarDeliveryDate").datepicker();
        //let datepicker = new DatePicker(document.getElementById('txtCarDeliveryDate'));
        //$('#txtCarDeliveryDate').click(function (e) {
        //    datepicker.showCalender();
        //});


    }
    else if (curPage.includes("/PRODUCTTERM") || curPage.includes("\\PRODUCTTERM")) {
        ...
        if (clientCode == 'EVSTAR') {
            $('#divWhatsCoveredCharger').show();
            $('#divWhatsCoveredPhone').hide();
            $('#divWhatsCoveredHome').hide();
        }
        else if (clientCode == 'DOB') {
            $('#divWhatsCoveredPhone').hide();
            $('#divWhatsCoveredCharger').hide();
            $('#divWhatsCoveredHome').show();
        }
        else {
            $('#divWhatsCoveredPhone').show();
            $('#divWhatsCoveredCharger').hide();
            $('#divWhatsCoveredHome').hide();
        }
                ...
                if (pass1.trim() != pass2.trim()) {
                    ...
                    $.ajax({
                        ...
                        headers: {
                            ...
                            "postalCode": zipCode, "phone": phone, "client": client.ID.toString(), "clientCode": clientCode
                        },
                        contentType: 'application/json; charset=utf-8',
                        success: function (response) {
                            if (response == null || (response.Result != null && response.Result.includes("NOTFOUND"))) {
        ...
        if (customer != null && customer.length > 0) {
            ...
            if (customer != null) {
                ...
                let url = '';
                if (method == "email") {
                    headers = { "address": customer.Email, "hashed": hashed, "code": code, "clientCode": customer.CustomerClient.Code };
                    url = "/api/category/0?program=" + customer.ProgramID;
                } else if (method == "sms") {
                    headers = { "phone": customer.MobileNumber, "hashed": hashed, "code": code, "clientCode": customer.CustomerClient.Code };
                    url = "/api/category/0?program=" + customer.ProgramID;
                }
                $.ajax({
                    type: 'GET',
        ...
        if (customer != null && customer.length > 0) {
            ...
            isReach = (customer.CustomerClient.Code == 'REACH');
            isDobson = (customer.CustomerClient.Code == 'DOB');
            isEVSTAR = (customer.CustomerClient.Code == 'EVSTAR');
                    ...
                    success: function (response) {
                        if (response == null) {
                            ...
                            if (products.length > 1) {
                                ...
                                $('#lblLandingTotalDevices').html('You have ' + products.length.toString() + ' registered device.');
                            }
                            else {
                            ...
                            for (var j = 0; j < products.length; j++) {
                                ...
                                $('#' + rowId).append('<a href="ProductAction.aspx?product=' + products[j].ID.toString() + '" style="width:195px;"><div id="' + colId + '" class="product-widget-selectable" />')
                            ...
                            if (customer.CustomerClient.Code != 'EVSTAR') {
        ...
        if (customer.CustomerClient.Code == 'DOB') {
            $('#dobsonlogo').show();
        }
        else {
            $('#dobsonlogo').hide();
                                ...
                                if (claims.length == 0) {
                                    ...
                                    for (var j = 0; j < claims.length; j++) {
                                        if (claims[j].CoveredProductID == productID) {
                                            ...
                                            if (claims[j].StatusHistory != null && claims[j].StatusHistory.length > 0) {
        ...
        $('#btnValidateCode').click(function (e) {
            ...
            if (regCode.trim() === '') {
                ...
                $.ajax({
                    type: 'GET',
                    url: "/api/customer",
                    headers: { "regCode": regCode, "clientCode": clientCode },
                    contentType: 'application/json; charset=utf-8',
                    success: function (response) {
                        if (response == null || response.ID === 0) {
                            showDialog('Sorry, we were unable to find your information. Please check your Registration Code and try again.', 'EVSTAR');
                        }
                        else {
                            if (response.StatusCode == 'Active') {
                                ...
                                $.ajax({
                                    ...
                                    success: function (response) {
                                        if (response == null || response.length == 0) {
                                            showDialog('Sorry, we are experiencing technical difficulties.');
                                        }
                                        else {
                                            client = response;
                                            if (client != null) {
        ...
        //}
        //else if (isEVSTAR) {
        $('#lnkRegistrationCode').show();
        $('btnRegisterProductText').html('Register your account');
        //}
        //else if (isReach) {
        //
        //}
        ...
        $('#btnSend2FA').click(function (e) {
            ...
            $.ajax({
                type: 'POST',
                url: url,
                headers: { "func": "CODE", "address": email2, "phone": customer.MobileNumber, "clientCode": clientCode },
                contentType: 'application/json; charset=utf-8',
                success: function (response) {
                    $('#divWait').hide();
                    if (response == null) {
        ...
        $('#btnSendVerification').click(function (e) {
            ...
            if (customer != null && customer != undefined && customer != 'undefined') {
                customer = JSON.parse(customer);
                let url = '';
                let selectedOption = $("input:radio[name=verification]:checked").val();
                let method = selectedOption;
                sessionStorage.setItem('method', method);
                url = "/api/" + selectedOption;
                $('#divWait').show();
                $.ajax({
                    type: 'POST',
                    url: url,
                    headers: { "func": "CODE", "address": customer.Email, "phone": customer.MobileNumber, "clientCode": customer.CustomerClient.Code },
                    contentType: 'application/json; charset=utf-8',
                    success: function (response) {
                        $('#divWait').hide();
                        if (response == null) {
        ...
        $('#btnSubmitDetails').click(function (e) {
            ...
            let clientCode = getClientCode();
            if (clientCode != null) {
                isReach = (clientCode == 'REACH');
                isDobson = (clientCode == 'DOB');
                isEVSTAR = (clientCode == 'EVSTAR');
            }
            ...
            if (productSelected != null) {
                ...
                if (clientProgram != null) {
                    ...
                    else if (productSelected.ProdCategory.FulfillmentType === 5) {  // The REACH Mobile hybrid fulfillment.
                        window.location.href = 'PhoneRepair.aspx';
                    }
                    else if (productSelected.ProdCategory.FulfillmentType === 3) { // In-Home
                        sessionStorage.setItem('repairOption', 'EVSE');
                        window.location.href = 'InHomeRepair.aspx';
                    }
                    else if (productSelected.ProdCategory.FulfillmentType === 6) { // Buyout
                        sessionStorage.setItem('productSelected', JSON.stringify(productSelected));
                        sessionStorage.setItem('repairOption', 'BUYOUT');
                        if (productSelected.ProdCategory.ServiceFee > 0.0)
        ...
        if (customer != null && customer != undefined && customer != 'undefined') {
            ...
            if (isReach || isEVSTAR) {
                ...
                if (productSelected != null) {
                    ...
                    if (productSelected.ProdCategory.FulfillmentType === 1) {  // Reimbursement or Buyout
                        $('#lblSubmitButton').text('Pay Service Fee');
                        $('#lblSubmitText').html('Pay Service Fee');
                        $('#spnSubmitButton').css('width', '124px');
                        $('#lblSubmitButton').css('width', '124px');
                    }
                    else if (productSelected.ProdCategory.FulfillmentType === 5) {  // The REACH Mobile hybrid fulfillment.
                        $('#lblSubmitButton').text('Submit');
                        $('#lblSubmitText').html('Submit');
                        ...
                        if (productSelected.ProdCategory != null && productSelected.ProdCategory.ServiceFee > 0.0) {
                            $('#lblSubmitButton').text('Pay Service Fee');
                            $('#lblSubmitText').html('Pay Service Fee');
                            $('#spnSubmitButton').css('width', '124px');
                            $('#lblSubmitButton').css('width', '124px');
                ...
                //$('#btnSubmitDetails').css('width', '180px');
            }
            else {
                $('#lblSubmitButton').text('Submit');
                $('#lblSubmitText').html('Submit');
        ...
        if (productCategory != null && productCategory != undefined && productCategory != 'undefined') {
            ...
            if (productCategory != null) {
                if (productCategory.FulfillmentType == 6) {  // buyout
                    if (productCategory.ServiceFee > 0.0) {
                        $('#divBuyout').hide();
                        $('#divReimburse').hide();
                        $('#divBuyoutServiceFee').show();
                    }
                    else {
                        $('#divBuyout').show();
                        $('#divReimburse').hide();
        ...
        $('#btnValidateInvoice').click(function () {
            ...
            if (email.trim() === '' || invoice.trim() === '') {
                ...
                $.ajax({
                    type: 'GET',
                    url: "/api/invoice",
                    headers: { "email": email, "invoice": invoice, "clientCode": clientCode },
                    contentType: 'application/json; charset=utf-8',
                    success: function (response) {
                        if (response == null) {
        ...
        $('#btnCollectPayment').click(function (e) {
            ...
            if (productSelected != null) {
                ...
                else if ($('#spnCollectPayment').html().toUpperCase().indexOf('CONTINUE') > -1) {
                    window.location.href = 'Reimbursement.aspx';
                }
                else {
                                        ...
                                        if ((cardType === 'AMEX' && cardCode > 999 && cardCode < 10000) ||
                                            ...
                                            if (cardZIPCode.length === 5) {
                                                ...
                                                $.ajax({
                                                    type: 'POST',
                                                    url: url,
                                                    headers: { "clientCode": customer.CustomerClient.Code },
                                                    contentType: 'application/json; charset=utf-8',
                                                                ...
                                                                if (response.messages != null) {
                                                                    if (invoice2 == null || invoice2 == undefined || invoice2.length === 0) {
                                                                        if (productSelected.ProdCategory != null && productSelected.ProdCategory.FulfillmentType === 3) { // In-Home
                                                                            if (customer.clientCode.Code == 'EVSTAR')
                                                                                sessionStorage.setItem('repairOption', 'EVSE')
                                                                            else
                                                                                sessionStorage.setItem('repairOption', 'IN-HOME');
                                                                            window.location.href = 'InHomeRepair.aspx';
                                                                            ...
                                                                            if (clientProgram != null) {
                                                                                if (productSelected.ProdCategory.FulfillmentType === 1) {  // Reimbursement only
                                                                                    $('#spnCollectPayment').html('Continue');
                                                                                }
                                                                                else
        ...
        if (program == 'EVSTAR') {
            ...
            repairOption = sessionStorage.getItem('repairOption');
            if (repairOption === 'LOCAL') {
                ...
                if (shippingLabel != null && shippingLabel.length > 0) {
                    ...
                    $('#lblShipping').html("www.fedex.com");
                }
            }
        }
        let customer = sessionStorage.getItem('Customer');
        ...
        $('#btnReset').click(function (e) {
            ...
            if (pw1 != pw2) {
                ...
                if (validatePassword(pw1)) {
                    $.ajax({
                        url: "/api/user/",
                        headers: { "username": email, "pwd": pw1, "clientCode": clientCode },
                        dataType: "json",
                        type: "PUT",
                        contentType: "application/json; charset=utf-8",
                        success: function (data) {
        ...
        $('#btnSendAdminLogin').click(function (e) {
            ...
            $.ajax({
                type: 'GET',
                url: "/api/user",
                headers: { "username": email, "auth": code, "clientCode": clientCode },
                contentType: 'application/json; charset=utf-8',
                success: function (response) {
                    response = JSON.parse(response);
        ...
        if (email.trim() === '' || pass1.trim() === '' || pass2.trim() === '') {
            ...
            if (pass1.trim() != pass2.trim()) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "/api/customer?function=REGISTER",
                    headers: { "email": email, "password": pass1, "confirm": pass2, "clientCode": clientCode },
                    contentType: 'application/json; charset=utf-8',
                    success: function (response) {
                        if (response == null || (response.Result != null && response.Result.includes("NOTFOUND"))) {
                            $('#lblError').html('Sorry, we were unable to find your information. Please check your entries and try again.');
                    ...
                    success: function (response) {
                        if (response == null || (response.Result != null && response.Result.includes("NOTFOUND"))) {
                            ...
                            $.ajax({
                                type: 'GET',
                                url: "/api/customer?function=LOGIN",
                                headers: { "email": email, "password": pass1, "clientCode": clientCode },
                                contentType: 'application/json; charset=utf-8',
                                success: function (response) {
                                    if (response == null || (response.Result != null && response.Result.includes("NOTFOUND"))) {
                                        $('#lblError').html('Sorry, we were unable to find your information using those credentials. Please check your entries and try again.');
...
function setUpProductIdentification() {
    ...
    $('#btnSaveDevice').click(function () {
        ...
        if (make.trim().length === 0 || model.trim().length === 0 || (productCategory.SerialNumberRequired && serial.trim().length === 0)) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response == null || response.length == 0) {
                        ...
                        let productCategory = response[0];
                        if (productCategory != null) {
                            if (productCategory.SerialNumberRequired)
                                $('#divNewProductSerial').show();
                        }
                    }
                },
                error: function (response) {
                    ...
                    if (response == null) {
                        ...
                        if (response.length > 0) {
                            ...
                            if (subcategorySelected != null)
                                $('#ddlSubcategories').val(subcategorySelected);
                            else
                                $('#ddlSubcategories').val(perils[0].SubcategoryID.toString());
                        }
...
function sendEmail(to, subject, body) {
    ...
    $.ajax({
        type: 'POST',
        url: url,
        headers: { "func": "MAIL", "subj": btoa(subject), "body": btoa(body), "clientCode": clientCode },
        contentType: 'application/json; charset=utf-8',
        success: function (response) {
            if (response == null) {
...
function uploadFile(file) {
    ...
    let product = sessionStorage.getItem('product');
    if (product != null && product != undefined && product.length > 0)
        url = 'FileProcess.aspx?product=' + product + '&clientCode=' + clientCode;
    else
...
function SendConfirmationEmail(customer, claim) {
    if (customer != null && customer != undefined && customer != 'undefined') {
        ...
        $.ajax({
            type: 'POST',
            url: url,
            headers: { "func": "EMAIL", "address": customer.Email, "clientCode": customer.CustomerClient.Code },
            contentType: 'application/json; charset=utf-8',
            success: function (response) {
                $('#divWait').hide();
                if (response == null) {
